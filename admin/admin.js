  ...
  // ===== Вкладки: Заказы (по умолчанию) / Редактор товаров / Корзина =====
  document.querySelectorAll(".admin-tab").forEach(tab => {
    tab.addEventListener("click", () => {
      ...
      // Корзина и полный каталог товаров грузятся не сразу при входе в панель
      // (см. showAdminInterface), а по первому клику на соответствующую вкладку —
      // оба используют свой кэш (trashCache/allProductsCache), так что повторные
      // клики не шлют запрос заново.
      if (tab.dataset.tab === "trash") refreshTrashList();
      if (tab.dataset.tab === "editor") refreshProductsList();
    });
  });
  ...
  function showAdminInterface() {
    ...
    trashCache = null;
    orderPhotoCache = new Map();
    // Полный каталог (~2979 товаров) и Корзину НЕ грузим сразу при входе —
    // это самое тяжёлое, что вообще делает панель, и оно нужно только на вкладке
    // "Редактор товаров"/"Корзина". По умолчанию открыта "Заказы" — она должна
    // становиться отзывчивой быстро, а не ждать своей очереди за тем, что ей
    // самой не нужно. Обе вкладки грузятся по первому клику, см. обработчик выше.
    // Фото товаров в самих заказах загружаются отдельно, точечно, только по тем
    // артикулам, что реально встретились в заказах — см. loadOrders().
    loadOrders();
  }
  ...
  // Товары, точечно подгруженные для миниатюр в заказах (см. loadOrders()) ДО того,
  // как открыли "Редактор товаров" и загрузился весь каталог. findProductBySku()
  // сначала смотрит в allProductsCache (когда он есть — это авторитетный источник),
  // и только потом сюда. Отдельная карта, а не запись прямо в allProductsCache: иначе
  // loadAllProducts() решил бы, что каталог уже когда-то полностью загружен, и не
  // прочитал бы остальные ~2900+ товаров при первом реальном открытии "Редактора".
  let orderPhotoCache = new Map();

  // Если "Редактор товаров" открыли дважды подряд быстро (или клик пришёлся на уже
  // идущую загрузку) — второе обращение просто ждёт то же самое чтение вместо того,
  // чтобы начинать своё параллельно.
  ...
  // сперва из allProductsCache (если "Редактор товаров" уже открывали — это полный,
  // авторитетный каталог), иначе из orderPhotoCache (точечная подгрузка специально
  // под заказы, см. loadOrders()). Если товар с тех пор удалили насовсем или
  // переименовали — карточка всё равно корректно откроется, просто без фото и названия.
  function findProductBySku(sku) {
    return (allProductsCache || []).find(p => p.sku === sku) || orderPhotoCache.get(sku) || null;
  }
  ...
  async function loadOrders() {
    ...
    try {
      ...
      render(); // сразу — без фото/названий, если товар ещё не в кэше (findProductBySku вернёт null)
      statusEl.textContent = `Показано: ${docs.length}`;

      // Фото/названия подтягиваем ТОЧЕЧНО — только по тем артикулам, что реально
      // встретились в этих заказах (обычно единицы-десятки), а не читая весь каталог
      // (~2979 товаров) целиком ради миниатюр. Раньше это (или полная загрузка
      // "Редактора" тем же чтением) было самым тяжёлым, что вообще делала панель
      // при каждом входе — и тормозило буквально всё, а не только заказы.
      const neededSkus = new Set();
      for (const d of docs) {
        for (const item of d.data().items || []) {
          if (item && item.sku && !findProductBySku(item.sku)) neededSkus.add(item.sku);
        }
      }
      if (neededSkus.size > 0) {
        Promise.all(
          [...neededSkus].map(sku => getDoc(doc(db, "products", sku)).catch(() => null))
        ).then(snaps => {
          let found = false;
          for (const snap of snaps) {
            if (snap && snap.exists()) {
              orderPhotoCache.set(snap.id, snap.data());
              found = true;
            }
          }
          if (found) render();
