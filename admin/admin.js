  ...
  document.querySelectorAll(".admin-tab").forEach(tab => {
    tab.addEventListener("click", () => {
      ...
      // Корзину открывают редко — грузим по первому клику, а не сразу при входе
      // в панель (см. showAdminInterface). refreshTrashList() сам использует
      // trashCache, так что повторные клики по вкладке не шлют запрос заново.
  ...
  function showAdminInterface() {
    ...
    refreshProductsList();
    loadOrders();
    // Корзину НЕ грузим сразу вместе с остальным: её открывают редко, а лёгкий
    // запрос (одно равенство deleted==true) стартовал бы в тот же момент, что и
    // тяжёлое чтение всего каталога товаров (~2979 документов) для "Редактора" —
    // и просто ждал бы своей очереди за ним, создавая видимость, что тормозит
    // именно Корзина, хотя сама она лёгкая. Та же причина, по которой loadOrders()
    // ниже не ждёт allProductsCache. Подгружается по клику на вкладку, см. ниже.
  }
